...
public class OrmLiteConfigUtil {
     ...
     */
    private int maxFindSourceLevel = 20;

    private final DatabaseType databaseType = new SqliteAndroidDatabaseType();

    /** A classloader used to load the classes that contains ORM Lite annotations. */
    ...
    public int getMaxFindSourceLevel() {
        return maxFindSourceLevel;
    }

    public void setMaxFindSourceLevel(int maxFindSourceLevel) {
        this.maxFindSourceLevel = maxFindSourceLevel;
    }
     ...
     * the raw folder.
     */
    public void writeConfigFile(String fileName) throws SQLException, IOException {
     ...
     */
    public void writeConfigFile(String fileName, Class<?>[] classes) throws SQLException, IOException {
     ...
     */
    public void writeConfigFile(File configFile) throws SQLException, IOException {
     ...
     */
    public void writeConfigFile(File configFile, File searchDir) throws SQLException, IOException {
        List<Class<?>> classList = new ArrayList<Class<?>>();
     ...
     */
    public void writeConfigFile(File configFile, Class<?>[] classes) throws SQLException, IOException {
     ...
     */
    public void writeConfigFile(OutputStream outputStream, File searchDir) throws SQLException, IOException {
        List<Class<?>> classList = new ArrayList<Class<?>>();
        findAnnotatedClasses(classList, searchDir, 0);
     ...
     * Write a configuration file to an output stream with the configuration for classes.
     */
    public void writeConfigFile(OutputStream outputStream, Class<?>[] classes) throws SQLException, IOException {
     ...
     */
    protected File findRawDir(File dir) {
    ...
    private void findAnnotatedClasses(List<Class<?>> classList, File dir, int level) throws SQLException,
        IOException {
        System.out.println("Looking for annotated classes in folder : " + dir.getAbsolutePath());
        ...
        for (File file : dir.listFiles()) {
            ...
            if (classHasAnnotations(clazz)) {
                System.out.println("Class: " + clazz.getName() + " detected.");
                classList.add(clazz);
            }
            ...
            try {
                for (Class<?> innerClazz : clazz.getDeclaredClasses()) {
                    if (classHasAnnotations(innerClazz)) {
                        System.out.println("Class: " + innerClazz.getName() + " detected.");
    ...
    private void writeConfigForTable(BufferedWriter writer, Class<?> clazz) throws SQLException, IOException {
    ...
    private boolean classHasAnnotations(Class<?> clazz) {
        while (clazz != null) {
            if (clazz.getAnnotation(getAnnotationClass(DatabaseTable.class)) != null) {
                return true;
            }
            Field[] fields;
            try {
                ...
                // amazingly, this sometimes throws an Error
                System.err.println("Could not load get declared fields from: " + clazz);
                System.err.println("     " + t);
                return false;
            }
            for (Field field : fields) {
                System.out.println("Database annotation :" + System.identityHashCode(DatabaseField.class));
                System.out.println("Database get annotation :" + System.identityHashCode(getAnnotationClass(DatabaseField.class)));
                for (Annotation a : field.getAnnotations()) {
                    System.out.println("Field annotation :" + System.identityHashCode(a.getClass()));
                }
                if (field.getAnnotation(getAnnotationClass(DatabaseField.class)) != null
                    || field.getAnnotation(getAnnotationClass(ForeignCollectionField.class)) != null) {
                    return true;
                }
            }
            try {
    ...
    private Class getAnnotationClass(Class<? extends Annotation> annotationClass) {
        if (classLoader == null) {
            return annotationClass;
        } else {
            try {
                return classLoader.loadClass(annotationClass.getName());
            } catch (Exception e) {
                System.err.println("Custom classloader could not load " + annotationClass.getName());
                return null;
            }
        }
    }
     ...
     */
    private String getPackageOfClass(File file) throws IOException {
     ...
     */
    private File findResRawDir(File dir) {
        for (File file : dir.listFiles()) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        new OrmLiteConfigUtil().writeConfigFile(args[0]);
    }
}
